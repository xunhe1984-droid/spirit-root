...
const exitOnError = (run) => (...args) => {
    try {
        return run(...args)
    } catch (error) {
        console.error(error)

        // Command callbacks run in a panic-recovering goroutine, so throwing
        // alone no longer propagates a nonzero status to the parent process.
        $os.exit(1)
    }
}
